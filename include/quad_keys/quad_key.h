...
class QuadKey {
 ...
 public:
  ...
  static QuadKey get_key_from_longitude_latitude_at_depth(
      Type t, const GeoCoordinate2d &coords, std::uint8_t depth);

  static std::vector<QuadKey> get_keys_from_longitude_latitude_bounding_box(
  ...
  bool operator==(const QuadKey &rhs) const {
    return m_row == rhs.m_row && m_col == rhs.m_col && m_type == rhs.m_type &&
           m_depth == rhs.m_depth;
  }

  bool operator!=(const QuadKey &rhs) const { return !((*this) == rhs); }

  friend std::ostream &operator<<(std::ostream &out, const QuadKey &obj) {
    return out << obj.type_as_string() << ", " << obj.get_row() << ", "
               << obj.get_col() << ", "
               << static_cast<std::uint32_t>(obj.get_depth());
  }
};
}  // namespace quad_keys
