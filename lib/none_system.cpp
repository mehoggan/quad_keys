...
namespace quad_keys {
bool NoneSystem::is_valid(const QuadKey & /*key*/) const { return false; }

bool NoneSystem::is_root_key(const QuadKey & /*self*/) const { return true; }
...
std::vector<QuadKey> NoneSystem::get_children(const QuadKey & /*self*/) const {
...
std::vector<QuadKey> NoneSystem::get_siblings(const QuadKey & /*self*/) const {
  return std::vector<QuadKey>();
}

std::vector<QuadKey> NoneSystem::get_neighbors(const QuadKey & /*self*/) const {
  return std::vector<QuadKey>();
}
...
std::vector<QuadKey> NoneSystem::get_keys_from_longitude_latitude_bounding_box(
    const GeoCoordinateBoundingBox2d & /*bounds*/,
    std::uint8_t /*depth*/) const {
  return std::vector<QuadKey>();
}

std::uint8_t NoneSystem::max_depth() const { return 0; }

std::uint32_t NoneSystem::max_rows(std::uint8_t /*depth*/) const { return 0; }

std::uint32_t NoneSystem::max_cols(std::uint8_t /*depth*/) const { return 0; }
}  // namespace quad_keys
