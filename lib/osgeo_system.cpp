...
GeoCoordinate2d meters_to_lat_lon(const MetersXY &mxy) {
  ...
  auto lat = inverse_mercator_tangent_half_angle_formula(mxy.y / r).val();
...
QuadKey OsGeoSystem::from_internal_string(const std::string &in_string) const {
  ...
  std::int32_t max_rows =
      to_int32(QuadKey::max_rows(Type::OsGeo, QuadKey::max_depth(Type::OsGeo)));
  std::int32_t max_cols =
      to_int32(QuadKey::max_cols(Type::OsGeo, QuadKey::max_depth(Type::OsGeo)));
...
std::vector<QuadKey> OsGeoSystem::get_keys_from_longitude_latitude_bounding_box(
  ...
  for (std::int32_t row = txyll.y; row <= txyur.y; ++row) {
    for (std::int32_t col = txyll.x; col <= txyur.x; ++col) {
      detail::insert_vector_if_valid_and_unique(
          quad_keys,
          QuadKey(Type::OsGeo, to_uint32(row), to_uint32(col), depth));
    }
  }
...
std::uint8_t OsGeoSystem::max_depth() const { return 22u; }
