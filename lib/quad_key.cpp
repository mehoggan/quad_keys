...
namespace quad_keys
{
  ...
  std::string QuadKey::type_as_string() const
  {
    return type_to_string(m_type);
  }

  bool QuadKey::is_root_key() const
  {
    return m_impl->is_root_key((*this));
  }

  QuadKey QuadKey::get_parent() const
  {
  ...
  std::vector<QuadKey>
  QuadKey::get_descendants(std::uint8_t depth_delta) const
  {
    std::vector<QuadKey> out_keys;
  ...
  QuadKey QuadKey::get_key_from_longitude_latitude_at_depth(Type t,
    const GeoCoordinate2d &coords, std::uint8_t depth)
  {
    ...
    QuadKey key = QuadKey(t)
      .m_impl->get_key_from_longitude_latitude_at_depth(coords, depth);
    return key;
  }

  std::vector<QuadKey>
  QuadKey::get_keys_from_longitude_latitude_bounding_box(
    Type t, const GeoCoordinateBoundingBox2d &bounds,
    std::uint8_t depth)
  {
    std::vector<QuadKey> out_keys;
    ...
    out_keys = QuadKey(t)
      .m_impl->get_keys_from_longitude_latitude_bounding_box(bounds, depth);
