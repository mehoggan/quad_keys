...
System &System::get_system_from_enum(Type t) {
  switch (t) {
    case Type::Bing:
      return bing_system;
    case Type::LatLon:
      return lat_lon_system;
    case Type::UlpQnr:
      return mos_qnr_system;
    case Type::OsGeo:
      return osgeo_system;
    case Type::Morton:
      return nds_system;
...
QuadKey System::get_ancestor(std::uint8_t depth_delta,
  ...
  if (static_cast<std::int8_t>(self.get_depth()) -
          static_cast<std::int8_t>(depth_delta) <
      0) {
    return QuadKey();
  }
...
QuadKey System::get_child(QuadKey::quadrant quad_id,
  ...
  if (self.get_type() == Type::None) {
    ...
    switch (quad_id) {
      case QuadKey::quadrant::north_west:
        row += 1u;
        break;
      case QuadKey::quadrant::north_east:
        row += 1u;
        col += 1u;
        break;
      case QuadKey::quadrant::south_east:
        col += 1u;
        break;
      case QuadKey::quadrant::south_west:
        break;
    }
...
std::vector<QuadKey> System::get_children(const QuadKey &self) const {
  ...
  for (std::uint8_t quad_id = 0; quad_id < 4; ++quad_id) {
    QuadKey child = get_child(static_cast<QuadKey::quadrant>(quad_id), self);
...
std::vector<QuadKey> System::get_descendants(std::uint8_t depth_delta,
                                             const QuadKey &self) const {
  std::vector<QuadKey> out_keys;
...
std::vector<QuadKey> System::get_neighbors(const QuadKey &self) const {
  ...
  if (self.is_root_key()) {
    ...
    uint32_t max_col_index =
        QuadKey::max_cols(self.get_type(), self.get_depth()) - 1;
    ...
    if (e.get_type() == Type::None) {
      ...
      if (n.get_type() == Type::None) {
        se = QuadKey(self.get_type(), self.get_row() - 1, 0, self.get_depth());
      } else {
        ne = QuadKey(self.get_type(), self.get_row() + 1, 0, self.get_depth());
      }
...
void System::get_geo_coordinate_bounds2d(GeoCoordinateBoundingBox2d &out_bounds,
                                         const QuadKey & /*self*/) const {
...
QuadKey System::from_internal_string(const std::string & /*in_string*/) const {
  return QuadKey();
}
...
std::vector<QuadKey> System::get_keys_from_longitude_latitude_bounding_box(
    const GeoCoordinateBoundingBox2d & /*bounds*/,
    std::uint8_t /*depth*/) const {
  return std::vector<QuadKey>();
}

std::uint8_t System::max_depth() const { return 0; }

uint32_t System::max_rows(std::uint8_t /*depth*/) const { return 0; }

uint32_t System::max_cols(std::uint8_t /*depth*/) const { return 0; }
}  // namespace quad_keys
