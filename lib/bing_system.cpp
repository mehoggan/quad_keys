...
Latitude inverse_mercator_formula_sinusoidal(double radians) {
  double latit = 90.0 - 360.0 * std::atan(std::exp(-radians * 2.0 * pi)) / pi;
...
UTile QuadKey_to_bing_tile(const QuadKey &key) {
  std::uint32_t max_index = QuadKey::max_rows(Type::Bing, key.get_depth()) - 1u;
...
void BingSystem::get_geo_coordinate_bounds2d(
  ...
  GeoCoordinate2d tl_coords = detail::pixel_to_lat_long(top_left_pixel, depth);

  UTile tmbr(tm.x + 1, tm.y + 1);
  UPixel bottom_right_pixel = detail::tile_to_pixel(tmbr);
  GeoCoordinate2d br_coords =
      detail::pixel_to_lat_long(bottom_right_pixel, depth);
...
QuadKey BingSystem::from_internal_string(const std::string &in_string) const {
...
std::vector<QuadKey> BingSystem::get_keys_from_longitude_latitude_bounding_box(
...
std::uint8_t BingSystem::max_depth() const { return 23u; }
