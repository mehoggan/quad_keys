...
namespace quad_keys
{
  ...
  QuadKey LatLonSystem::from_internal_string(
    const std::string &in_string) const
  {
    ...
    for (auto c : in_string) {
      switch (c) {
      case '2': ret = ret.get_child(QuadKey::quadrant::south_west); break;
      case '3': ret = ret.get_child(QuadKey::quadrant::south_east); break;
      case '0': ret = ret.get_child(QuadKey::quadrant::north_west); break;
      case '1': ret = ret.get_child(QuadKey::quadrant::north_east); break;
      }
    }
    return ret;
  }
  ...
  std::uint8_t LatLonSystem::max_depth() const
  {
    return 23u;
  }
