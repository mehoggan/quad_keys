...
namespace quad_keys
{
  ...
  {
    EXPECT_EQ(QuadKey(),
      QuadKey().get_child(QuadKey::quadrant::north_west));
    uint8_t max_depth_bing = QuadKey::max_depth(Type::Bing);
    EXPECT_EQ(QuadKey(), QuadKey(Type::Bing, 0, 0,
      max_depth_bing).get_child(QuadKey::quadrant::north_east));
  ...
  TEST(BINGSYSTEM, get_children)
  {
    ...
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 0, 0, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 0, 1, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 1, 0, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 1, 1, 2)), test_keys.end());

    test_keys = QuadKey(Type::Bing, 0, 1, 1).get_children();
    EXPECT_EQ(4u, test_keys.size());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 0, 2, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 0, 3, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 1, 2, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 1, 3, 2)), test_keys.end());

    test_keys = QuadKey(Type::Bing, 1, 0, 1).get_children();
    EXPECT_EQ(4u, test_keys.size());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 2, 0, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 2, 1, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 3, 0, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 3, 1, 2)), test_keys.end());

    test_keys = QuadKey(Type::Bing, 1, 1, 1).get_children();
    EXPECT_EQ(4u, test_keys.size());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 2, 2, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 2, 3, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 3, 2, 2)), test_keys.end());
    EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      QuadKey(Type::Bing, 3, 3, 2)), test_keys.end());
  ...
  TEST(BINGSYSTEM, get_descendants)
  {
...

    {
      ...
      EXPECT_EQ(4u, test_keys.size());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 0, 1)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 1, 1)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 0, 1)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 1, 1)), test_keys.end());
      ...
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 0, 7, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 1, 7, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 2, 7, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 3, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      ...
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 4, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 4, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 4, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 4, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
      ...
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 5, 7, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 6, 7, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 0, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 1, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 2, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 3, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 4, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 5, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 6, 3)), test_keys.end());
      EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
        QuadKey(Type::Bing, 7, 7, 3)), test_keys.end());
    }
  }
  ...
  TEST(BINGSYSTEM, SelectedCities)
  {
    ...
    { // Chicago 41.822, -87.635
      ...
      key = QuadKey::get_key_from_longitude_latitude_at_depth(t,
        coords, depth);
