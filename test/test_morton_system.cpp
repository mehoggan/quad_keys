...
TEST(MORTONSYSTEM, get_neighbors) {
  ...
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 0, 1, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 1, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 0, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 0, 7, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 7, 2)),
            test_keys.end());
  test_keys.clear();
  ...
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
            ...
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 2, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 0, 2, 2)),
            test_keys.end());
  test_keys.clear();
  ...
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
            ...
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 3, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 0, 3, 2)),
            test_keys.end());
  test_keys.clear();
  ...
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
            ...
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 1, 4, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 0, 4, 2)),
            test_keys.end());
  test_keys.clear();
  ...
  EXPECT_EQ(5u, test_keys.size());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 3, 0, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 2, 0, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 2, 1, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 2, 2, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 3, 2, 2)),
            test_keys.end());
  test_keys.clear();
  ...
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
            ...
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 2, 3, 2)),
            test_keys.end());
  EXPECT_NE(std::find(test_keys.begin(), test_keys.end(),
                      QuadKey(Type::Morton, 3, 3, 2)),
            test_keys.end());
  test_keys.clear();
...
TEST(MORTONSYSTEM, from_string_to_string) {
  ...
  class QuadKey quad_key_back =
      QuadKey::from_internal_string(Type::Morton, fmt);
...
TEST(MORTONSYSTEM, get_key_from_longitude_latitude_at_depth_to_bounds) {
  ...
  {  // West
    ...
    EXPECT_TRUE(GeoCoordinate2d(Longitude(-180.0), Latitude(-90.0))
                    .near(actual.lower_left(), 1))
        << "actual = " << actual.lower_left();
    EXPECT_TRUE(GeoCoordinate2d(Longitude(-0.1), Latitude(+90.0))
                    .near(actual.upper_right(), 1))
  ...
  {  // East
    ...
    GeoCoordinateBoundingBox2d actual;
    morton_key.get_geo_coordinate_bounds2d(actual);
    EXPECT_TRUE(GeoCoordinate2d(Longitude(+0.0), Latitude(-90.0))
                    .near(actual.lower_left(), 1))
        << "actual = " << actual.lower_left();
    EXPECT_TRUE(GeoCoordinate2d(Longitude(+180.0), Latitude(+90.0))
                    .near(actual.upper_right(), 1))
  ...
  for (const auto &pair :
    ...
    QuadKey morton_key;
    morton_key = QuadKey::get_key_from_longitude_latitude_at_depth(Type::Morton,
                                                                   ll, depth);
    EXPECT_EQ(pair.second, morton_key.to_internal_string())
        << "ll: " << pair.second << " " << pair.first;
    morton_key = QuadKey::get_key_from_longitude_latitude_at_depth(Type::Morton,
                                                                   lr, depth);
    EXPECT_EQ(pair.second, morton_key.to_internal_string())
        << "lr: " << pair.second << " " << pair.first;
    morton_key = QuadKey::get_key_from_longitude_latitude_at_depth(Type::Morton,
                                                                   ur, depth);
    EXPECT_EQ(pair.second, morton_key.to_internal_string())
        << "ur: " << pair.second << " " << pair.first;
    morton_key = QuadKey::get_key_from_longitude_latitude_at_depth(Type::Morton,
                                                                   ul, depth);
    EXPECT_EQ(pair.second, morton_key.to_internal_string())
        << "ul: " << pair.second << " " << pair.first;
    GeoCoordinateBoundingBox2d actual;
    morton_key.get_geo_coordinate_bounds2d(actual);
...
TEST(MORTONSYSTEM, get_keys_from_longitude_latitude_bounding_box) {
  for (const auto &data : {
    ...
    for (const QuadKey &quad_key : data.second) {
      EXPECT_NE(std::find(data.second.begin(), data.second.end(), quad_key),
                data.second.end());
    }
  }
  for (const auto &data : {
...
TEST(MORTONSYSTEM, selected_cities) {
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(+2.29450000000000),
                                           Latitude(+48.85822200000000)));
    std::uint32_t tile =
        8428778 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
    EXPECT_EQ(ss.str(), morton_quad_key.to_internal_string());
  }
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(-74.04444400000000),
                                           Latitude(+40.68916700000000)));
    std::uint32_t tile =
        86924190 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(-43.15744400000000),
                                           Latitude(-22.94865800000000)));
    std::uint32_t tile =
        130726287 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(+151.21527800000000),
                                           Latitude(-33.85611100000000)));
    std::uint32_t tile =
        63372939 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(+0.00000000000000),
                                           Latitude(+51.50300000000000)));
    std::uint32_t tile =
        8521770 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(13, GeoCoordinate2d(Longitude(-78.45000000000000),
                                           Latitude(+0.00000000000000)));
    std::uint32_t tile =
        84148305 | (1 << (static_cast<std::uint8_t>(data.first) + 16));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(+2.29450000000000),
                                           Latitude(+48.85822200000000)));
    std::uint32_t tile =
        131699u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(-74.04444400000000),
                                           Latitude(+40.68916700000000)));
    std::uint32_t tile =
        1358190u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(-43.15744400000000),
                                           Latitude(-22.94865800000000)));
    std::uint32_t tile =
        2042598u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(+151.21527800000000),
                                           Latitude(-33.85611100000000)));
    std::uint32_t tile =
        990202u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(+0.00000000000000),
                                           Latitude(+51.50300000000000)));
    std::uint32_t tile =
        133152u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
  ...
  {
    auto data =
        std::make_pair(10, GeoCoordinate2d(Longitude(-78.45000000000000),
                                           Latitude(+0.00000000000000)));
    std::uint32_t tile =
        1314817u | (1 << (16 + static_cast<std::uint8_t>(data.first)));
    std::stringstream ss;
    ss << tile;
    QuadKey morton_quad_key = QuadKey::get_key_from_longitude_latitude_at_depth(
        Type::Morton, data.second, static_cast<std::uint8_t>(data.first));
