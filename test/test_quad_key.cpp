...
TEST(QUADKEY, system_specific_ctor) {
  auto types = {Type::Bing,   Type::LatLon, Type::UlpQnr,
                Type::Morton, Type::None,   Type::OsGeo};
  for (auto type : types) {
    for (uint8_t depth = 0; depth < 10; ++depth) {
      ...
      EXPECT_EQ(QuadKey(type, max_rows - 1, max_cols - 1, depth), valid)
          << (std::string("Failed valid check on type ") + type_str);
    }
  }
}
}  // namespace quad_keys
